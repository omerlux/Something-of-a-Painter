...
def discriminator_fn_prehead(hieght, width, channels):
    initializer = tf.random_normal_initializer(0., 0.02)
    gamma_init = tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02)
    ...
    return Model(inputs=inp, outputs=zero_pad2)


def discriminator_head():
    initializer = tf.random_normal_initializer(0., 0.02)

    inp = L.Input(shape=[33, 33, 512], name='input_image')
    x = inp

    last = L.Conv2D(1, 4, strides=1, kernel_initializer=initializer)(x)  # (size, 30, 30, 1)
